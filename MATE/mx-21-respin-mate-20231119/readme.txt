...
Note:
mx-21-respin-mate-xxxxxxxx.iso ----- Default language (locale) is set to English. You can choose another one from among pull-down list.

You are free to choose whatever locale you want to use at Boot Menu Option. And also, to change another locale after system booting, logout a present desktop session. Then, click on the upper right corner on the login screen, and choose a locale from a drop-down list. Simply logout and then login again can change a locale for a desktop session.
