2023-02-03 09:00 UTC
updated: 02-05 08:05 UTC
...
A snapshot is modified thoroughly from MX-21.3_minimal_x64 Wildflower 15 Jan 2023, and applied all latest updates so far.
This updated version is now shown as "mx-respin-iso-21.3_x64 (February 03, 2023).
...
**Known bugs
Each MX Tool should start from commandline, as it might NOT start up from Main Menu. In order to know the command of an MX application, right-click the menu > Edit Launcher > Command.
To fix it, please install LXDE PolicyKit as below:

 apt install lxpolkit

Audio is muted by default (this is not a bug).
...
MX-21.3_minimal_x64 Wildflower 15 Jan 2023
...
Note:
xxxxxx.iso is identical to xxxxxx-ja.iso besides "boot parameter" below.

xxxxxx.iso ----- Nn default locale is set. You should choose from among pull-down list.
xxxxxx-ja.iso -- lang=ja_JP kbd=us tz=Asia/Tokyo
