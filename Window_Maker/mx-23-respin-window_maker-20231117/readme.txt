2023-08-28 13:30 UTC
updated: 2023-11-17 02:04 UTC
...
** Known issus
Applications which need root privilege such as Synaptic, MX Snapshot and MX Live-USB Maker can not start from Desktop menu, so run commandline instead.
For example:
<app name> --> <command>
Synaptic --> synaptic (as user)
MX Snapshot --> mx-snapshot -c
MX LiveUSB Maker --> live-usb-maker
MX Package Installer --> cli-aptiX (Note: "X" is uppercase.)
...
Modified from Cmmunity respin of
...
Note:
As for mx-23-respin-window_maker-xxxxxx.iso, No default locale is set. So you should choose from among pull-down list.
